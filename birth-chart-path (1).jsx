...
/* Birth Chart Path v3
   Tokens + Stripe + AI Chat + Account System
...
// STRIPE
const STRIPE_LINK="https://buy.stripe.com/4gM3cn8oV8M57Wv4GL9Ve00";

// Token readings
const READINGS=[
  {id:"deep",name:"Deep Chart Reading",icon:"\u{1F52E}",cost:3,desc:"A detailed multi-paragraph analysis of your complete birth chart — Sun, Moon, Rising, and all planetary placements."},
  {id:"love",name:"Love Forecast",icon:"\u2764\uFE0F",cost:3,desc:"What the stars say about your romantic life this month. Who you attract, what to watch for, and when to make a move."},
  {id:"career",name:"Career Path Analysis",icon:"\u{1F4BC}",cost:3,desc:"Your cosmic career blueprint. Where your chart says you'll thrive, and what's coming professionally."},
  {id:"year",name:"Year Ahead Forecast",icon:"\u{1F30D}",cost:5,desc:"A comprehensive look at your next 12 months — key dates, opportunities, challenges, and turning points."},
  {id:"past",name:"Past Life Reading",icon:"\u{1F54A}\uFE0F",cost:5,desc:"What your South Node and Saturn placement reveal about your soul's journey and karmic patterns."},
  {id:"shadow",name:"Shadow Self Reading",icon:"\u{1F311}",cost:4,desc:"The hidden parts of your chart — your Pluto, 12th house energy, and the patterns you repeat unconsciously."}
];
const PACKS=[{tokens:5,price:"$4.99",pop:false},{tokens:15,price:"$9.99",pop:true},{tokens:30,price:"$17.99",pop:false}];

const hashPw=s=>{let h=0;for(let i=0;i<s.length;i++){h=((h<<5)-h)+s.charCodeAt(i);h|=0}return h.toString(36)};
const LS={get:k=>{try{return JSON.parse(localStorage.getItem("bcp_"+k))}catch{return null}},set:(k,v)=>{try{localStorage.setItem("bcp_"+k,JSON.stringify(v))}catch{}},del:k=>{try{localStorage.removeItem("bcp_"+k)}catch{}}};
...
const CityInput=memo(({inputRef})=>{
  ...
  return <div ref={wr} style={{position:"relative"}}><input value={q} onChange={e=>{const v=e.target.value;setQ(v);if(inputRef)inputRef.current=v;setRes(citySearch(v));setShow(v.length>=2)}} placeholder="City (100+ US cities)" style={sInp}/>{show&&res.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:50,background:C.card,border:"1px solid "+C.bor,borderRadius:8,marginTop:4,maxHeight:200,overflowY:"auto"}}>{res.map(c=><div key={c} onClick={()=>{setQ(fmtCity(c));if(inputRef)inputRef.current=fmtCity(c);setShow(false)}} style={{padding:"8px 12px",fontFamily:F,fontSize:12,color:C.tx,cursor:"pointer",borderBottom:"1px solid "+C.bor+"30"}}>{fmtCity(c)}</div>)}</div>}</div>;
});

export default function BirthChartPath(){
  const[view,setView]=useState("home");
  const[user,setUser]=useState(()=>LS.get("user"));
  const[auth,setAuth]=useState(false);
  const[authMode,setAuthMode]=useState("signup");
  const[rd,setRd]=useState(null);const[rl,setRl]=useState(false);
  ...
  const[aNm,setANm]=useState("");const[aEm,setAEm]=useState("");const[aPw,setAPw]=useState("");
  const[aMo,setAMo]=useState("");const[aDy,setADy]=useState("");const[aYr,setAYr]=useState("");
  const[aHr,setAHr]=useState("12");const[aAp,setAAp]=useState("PM");const acity=useRef("");
  ...
  const updUser=patch=>{const u={...user,...patch};setUser(u);LS.set("acct_"+u.email,u)};

  const doSignup=()=>{
    setAuthErr("");
    if(!aNm.trim())return setAuthErr("Enter your name");
    if(!aEm||!aEm.includes("@"))return setAuthErr("Enter a valid email");
    if(!aPw||aPw.length<6)return setAuthErr("Password must be 6+ characters");
    if(!aMo||!aDy)return setAuthErr("Enter your birthday");
    const existing=LS.get("acct_"+aEm.trim().toLowerCase());
    if(existing)return setAuthErr("Account already exists. Log in instead.");
    ...
    const acct={name:aNm.trim(),email:aEm.trim().toLowerCase(),pwHash:hashPw(aPw),signKey:ch.sun.sign,chart:ch,bday:getBday(mi,d),mo:mi,dy:d,tokens:3,createdAt:Date.now()};
    LS.set("acct_"+acct.email,acct);setUser(acct);setAuth(false);
    setANm("");setAEm("");setAPw("");setAMo("");setADy("");setAYr("");setAuthErr("");
  };
  const doLogin=()=>{
    setAuthErr("");
    if(!aEm||!aPw)return setAuthErr("Enter email and password");
    const acct=LS.get("acct_"+aEm.trim().toLowerCase());
    if(!acct)return setAuthErr("No account found");
    if(acct.pwHash!==hashPw(aPw))return setAuthErr("Wrong password");
    setUser(acct);setAuth(false);setAEm("");setAPw("");setAuthErr("");
  };
  const doEmailOnly=()=>{
    setAuthErr("");if(!aEm||!aEm.includes("@"))return;
    const emails=LS.get("email_list")||[];
    if(!emails.includes(aEm.trim().toLowerCase()))emails.push(aEm.trim().toLowerCase());
    LS.set("email_list",emails);setAEm("");
  };
  const doLogout=()=>{setUser(null);LS.del("user");go("home")};
  ...
  const doRead=()=>{
    ...
    setTimeout(()=>{
      ...
      let lat=33.45,lng=-112.07;for(const[k,co]of Object.entries(CITIES)){if(k.replace(/,/g," ")===loc||loc.includes(k.split(",")[0])){lat=co[0];lng=co[1];break}}
      const ch=birthChart(y,mi,d,hr,lat,lng);
      setRd({sKey:ch.sun.sign,ch,mi,d,y,bday:getBday(mi,d)});setRl(false);
    },800);
  };

  const callAI=async(prompt,sys)=>{
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:sys||"",messages:[{role:"user",content:prompt}]})});
    const j=await r.json();return j.content?.map(b=>b.text||"").join("")||"The stars are quiet right now. Try again soon.";
  };

  const sendChat=async()=>{
    if(!chatIn.trim()||chatLoading)return;
    const msg=chatIn.trim();setChatIn("");
    setChatMsgs(prev=>[...prev,{role:"user",text:msg}]);setChatLoading(true);
    try{
      const sys="You are the Birth Chart Path astrologer. User: "+user.name+" (Sun "+user.signKey+", Moon "+user.chart.moon.sign+", Rising "+user.chart.rising.sign+"). Moon: "+moon.name+" "+moon.pct+"%. Be warm, insightful, mystical but grounded. Under 150 words.";
      const history=chatMsgs.slice(-6).map(m=>({role:m.role==="user"?"user":"assistant",content:m.text}));
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:sys,messages:[...history,{role:"user",content:msg}]})});
      const j=await r.json();const reply=j.content?.map(b=>b.text||"").join("")||"The cosmic connection is faint. Try again.";
      setChatMsgs(prev=>[...prev,{role:"assistant",text:reply}]);
    }catch{setChatMsgs(prev=>[...prev,{role:"assistant",text:"The stars wavered. Please try again."}])}
    setChatLoading(false);
  };

  const buyReading=async(rd)=>{
    if(!user)return;
    if(tokens<rd.cost)return;
    updUser({tokens:tokens-rd.cost});
    setActiveReading(rd);setReadingResult(null);setReadingLoading(true);
    const prompts={
      deep:"Give a detailed birth chart reading for "+user.name+". Sun in "+user.signKey+", Moon in "+user.chart.moon.sign+", Rising in "+user.chart.rising.sign+", Mercury in "+user.chart.mercury.sign+", Venus in "+user.chart.venus.sign+", Mars in "+user.chart.mars.sign+". Explain how these placements interact. 4 paragraphs, 300 words.",
      love:"Give a love and relationship forecast for "+user.name+" with Sun "+user.signKey+", Venus in "+user.chart.venus.sign+", Mars in "+user.chart.mars.sign+", Moon in "+user.chart.moon.sign+". What do they attract? What patterns do they repeat? What's ahead romantically? 3 paragraphs, 250 words.",
      career:"Give career and purpose guidance for "+user.name+" with Sun "+user.signKey+", Saturn in "+user.chart.saturn.sign+", Jupiter in "+user.chart.jupiter.sign+", Mercury in "+user.chart.mercury.sign+". What are they built for? Where will they thrive? 3 paragraphs, 250 words.",
      year:"Write a year-ahead forecast for "+user.name+" (Sun "+user.signKey+", Moon "+user.chart.moon.sign+", Rising "+user.chart.rising.sign+"). Cover love, career, health, and growth. Mention specific months. 4 paragraphs, 300 words.",
      past:"Give a past life and karmic reading for "+user.name+" based on Sun "+user.signKey+", Moon "+user.chart.moon.sign+", Saturn in "+user.chart.saturn.sign+". What karmic lessons are they working through? What past life patterns emerge? 3 paragraphs, 250 words. Be mystical.",
      shadow:"Give a shadow self reading for "+user.name+" with Sun "+user.signKey+", Moon "+user.chart.moon.sign+", Mars in "+user.chart.mars.sign+", Saturn in "+user.chart.saturn.sign+". Explore the hidden patterns, unconscious drives, and self-sabotage tendencies in their chart. 3 paragraphs, 250 words. Be honest but compassionate."
    };
    try{
      const text=await callAI(prompts[rd.id],"You are a gifted professional astrologer for Birth Chart Path. Write personalized readings that feel like a real consultation. Be specific, psychologically insightful, and grounded.");
      setReadingResult(text);
    }catch{setReadingResult("The stars need a moment. Your tokens have been refunded.");updUser({tokens:tokens})}
    setReadingLoading(false);
  };

  useEffect(()=>{if(chatEnd.current)chatEnd.current.scrollIntoView({behavior:"smooth"})},[chatMsgs]);

  const css="@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&display=swap');*{box-sizing:border-box;-webkit-tap-highlight-color:transparent}body{margin:0;overflow-x:hidden}::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#30363D;border-radius:2px}select{color-scheme:dark}input::placeholder{color:#484F58}button{transition:transform 80ms}button:active{transform:scale(.97)}@keyframes spin{to{transform:rotate(360deg)}}@keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.fade{animation:fadeIn .3s ease both}@keyframes pulse{0%,100%{opacity:.4}50%{opacity:1}}.typing{animation:pulse 1.2s ease infinite}";

  return <div style={{background:C.bg,minHeight:"100vh",color:C.tx,position:"relative"}}><style>{css}</style>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,height:52,background:C.bg+"EE",backdropFilter:"blur(16px)",borderBottom:"1px solid "+C.bor,padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        {user&&<div style={{display:"flex",alignItems:"center",gap:3,padding:"4px 10px",background:C.gold+"12",borderRadius:20,border:"1px solid "+C.gold+"25",cursor:"pointer"}} onClick={()=>go("tokens")}>
          <span style={{fontSize:11}}>{"\u2B50"}</span>
          <span style={{fontFamily:F,fontSize:11,fontWeight:700,color:C.gold}}>{tokens}</span>
        </div>}
        {user?<button onClick={()=>go("profile")} style={{background:C.soft,border:"1.5px solid "+C.bor,borderRadius:7,width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontFamily:F,fontSize:11,fontWeight:700,color:C.tx}}>{user.name[0]}</button>
          :<div style={{display:"flex",gap:6}}>
            <button onClick={()=>{setAuthMode("login");setAuth(true)}} style={{fontFamily:F,fontSize:11,fontWeight:600,background:"transparent",color:C.med,border:"none",cursor:"pointer"}}>Log in</button>
            <button onClick={()=>{setAuthMode("signup");setAuth(true)}} style={{fontFamily:F,fontSize:12,fontWeight:600,background:C.pri,color:C.bg,border:"none",borderRadius:8,padding:"7px 14px",cursor:"pointer"}}>Sign up</button>
          </div>}
      </div>
    </nav>
    ...
    <nav style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:C.bg+"F0",backdropFilter:"blur(16px)",borderTop:"1px solid "+C.bor,display:"flex",justifyContent:"space-around",padding:"5px 0 8px"}}>
      {[["home","Home"],["reading","Chart"],["chat","AI Chat"],["tokens","Readings"],["learn","Learn"]].map(([v,l])=>
    ...
    {view==="home"&&<div className="fade">
      <div style={{padding:"28px 0 20px",textAlign:"center"}}>
        ...
        <h1 style={{fontFamily:F,fontSize:"clamp(21px,6.5vw,27px)",fontWeight:700,lineHeight:1.2,margin:"0 0 8px",letterSpacing:"-.04em"}}>Your real birth chart.<br/>AI-powered readings.</h1>
        <p style={{fontFamily:F,fontSize:13,color:C.med,lineHeight:1.5,margin:"0 auto 18px",maxWidth:360}}>Astronomical calculations. 366 unique birthday personalities. Personalized AI horoscopes.</p>
        <div style={{display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap"}}>
          <button style={{...sBtn,width:"auto",padding:"12px 22px"}} onClick={()=>go("reading")}>Calculate your chart</button>
          {!user&&<button onClick={()=>{setAuthMode("signup");setAuth(true)}} style={{fontFamily:F,fontWeight:600,border:"1px solid "+C.pri+"25",cursor:"pointer",borderRadius:10,background:"transparent",color:C.pri,padding:"12px 22px",fontSize:13}}>Create account</button>}
        </div>
      </div>

      {todayBday&&<div style={{...sCard,background:"linear-gradient(135deg,"+C.card+" 0%,#1a1f2e 100%)"}}><div style={sLbl}>Today's Birthday Personality</div><h3 style={{fontFamily:F,fontSize:16,fontWeight:700,color:C.gold,margin:"4px 0 6px"}}>{todayBday}</h3><div style={{fontFamily:F,fontSize:10,color:C.dim,marginTop:4}}>{now.toLocaleDateString("en-US",{month:"long",day:"numeric"})}</div></div>}
      {todayHist&&<div style={sCard}><div style={sLbl}>{"\u2B50 Born Today"}</div><div style={{fontFamily:F,fontSize:13}}>{todayHist}</div></div>}
      ...
      {user&&uSign&&<div style={{...sCard,boxShadow:"0 0 20px rgba(88,166,255,0.08)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:3,padding:"4px 10px",background:C.gold+"12",borderRadius:20}}><span style={{fontSize:10}}>{"\u2B50"}</span><span style={{fontFamily:F,fontSize:11,fontWeight:700,color:C.gold}}>{tokens} tokens</span></div>
        </div>
      ...
      <div style={{...sCard,cursor:cl?"default":"pointer"}} onClick={()=>{if(!cl)setCl(true)}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div><div style={{fontFamily:F,fontSize:13,fontWeight:700}}>{cl?"Claimed":"Daily insight ready"}</div><div style={{fontFamily:F,fontSize:11,color:C.med}}>{cl?"+50 XP":"Tap to collect"}</div></div>
        </div>
      </div>
      ...
      <div style={sCard}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}><div style={{fontFamily:F,fontSize:13,fontWeight:700}}>{moon.name}</div><div style={{fontFamily:F,fontSize:11,color:C.med}}>{moon.msg}</div><div style={{fontFamily:F,fontSize:10,color:C.dim,marginTop:2}}>Day {moon.day} of 29.5</div></div>
          <div style={{textAlign:"right"}}><div style={{fontFamily:F,fontSize:18,fontWeight:800,color:C.pri}}>{moon.pct}%</div><div style={{fontFamily:F,fontSize:9,color:C.dim}}>illuminated</div></div>
        </div>
      </div>

      <div style={sCard}><div style={sLbl}>The Zodiac</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {Object.entries(SIGNS).map(([k,s])=><div key={k} onClick={()=>{setLSign(k);go("learn")}} style={{textAlign:"center",padding:"10px 4px",borderRadius:8,background:EL[s.el].color+"08",border:"1px solid "+EL[s.el].color+"15",cursor:"pointer"}}><div style={{fontSize:18}}>{s.sym}</div><div style={{fontFamily:F,fontSize:10,fontWeight:600}}>{k}</div></div>)}
        </div>
      </div>

      {!user&&<div style={{...sCard,border:"1.5px solid "+C.pri+"30",textAlign:"center"}}>
        <div style={{fontFamily:F,fontSize:14,fontWeight:700,marginBottom:4}}>Stay on your path</div>
        <p style={{fontFamily:F,fontSize:11,color:C.med,margin:"0 0 12px"}}>Get weekly cosmic insights delivered to your inbox.</p>
        <div style={{display:"flex",gap:6}}><input value={aEm} onChange={e=>setAEm(e.target.value)} placeholder="Your email" style={{...sInp,flex:1}}/><button onClick={()=>{doEmailOnly()}} style={{...sBtn,width:"auto",padding:"10px 16px",fontSize:12}}>Join</button></div>
      </div>}
    </div>}
    ...
    {view==="reading"&&<div className="fade">
      <div style={{...sCard,boxShadow:"0 0 20px rgba(88,166,255,0.08)"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}><select defaultValue="" onChange={e=>{rmo.current=e.target.value}} style={sSel}><option value="">Month</option>{MO.map(m=><option key={m}>{m}</option>)}</select><select defaultValue="" onChange={e=>{rdy.current=e.target.value}} style={sSel}><option value="">Day</option>{Array.from({length:31},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select></div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}><select defaultValue="12" onChange={e=>{rhr.current=e.target.value}} style={sSel}>{Array.from({length:12},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select><select defaultValue="PM" onChange={e=>{rap.current=e.target.value}} style={{...sSel,maxWidth:80}}><option value="AM">AM</option><option value="PM">PM</option></select></div>
        <CityInput inputRef={rloc}/><p style={{fontFamily:F,fontSize:9,color:C.dim,margin:"6px 0 12px"}}>Birth time affects Rising sign.</p>
      ...
      {rd&&!rl&&<div className="fade">
        {rd.bday&&<div style={{...sCard,background:"linear-gradient(135deg,"+C.card+" 0%,#1a1f2e 100%)"}}><div style={sLbl}>{"Your Birthday Personality \u00B7 "+MO[rd.mi-1]+" "+rd.d}</div><h3 style={{fontFamily:F,fontSize:18,fontWeight:700,color:C.gold,margin:"4px 0"}}>{rd.bday}</h3></div>}
        <div style={{...sCard,boxShadow:"0 0 20px rgba(88,166,255,0.08)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><span style={{fontSize:28}}>{SIGNS[rd.sKey].sym}</span><div><h3 style={{fontFamily:F,fontSize:18,fontWeight:700,margin:0}}>{SIGNS[rd.sKey].tag}</h3><div style={{display:"flex",gap:5,marginTop:3,flexWrap:"wrap"}}><span style={sTag()}>{SIGNS[rd.sKey].sym+" "+rd.sKey}</span><span style={sTag(EL[SIGNS[rd.sKey].el].color)}>{EL[SIGNS[rd.sKey].el].icon+" "+EL[SIGNS[rd.sKey].el].name}</span></div></div></div>
          <div style={sSoft}><p style={{fontFamily:F,fontSize:14,fontWeight:500,fontStyle:"italic",lineHeight:1.6,margin:"0 0 8px"}}>{SIGNS[rd.sKey].line}</p><p style={{fontFamily:F,fontSize:13,lineHeight:1.65,margin:0}}>{SIGNS[rd.sKey].reading}</p></div>
        ...
        <div style={sCard}><div style={sLbl}>Complete Birth Chart</div>
          {Object.entries(PI).map(([k,p])=>{const pl=rd.ch[k];if(!pl)return null;const s=SIGNS[pl.sign];const e=EL[s?.el]||EL.fire;return <div key={k} style={{background:e.color+"06",borderRadius:10,padding:12,border:"1px solid "+e.color+"15",marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{p.i}</span><span style={{fontFamily:F,fontSize:13,fontWeight:700}}>{p.n+" in "+pl.sign}</span><span style={{fontFamily:F,fontSize:10,color:C.dim}}>{pl.deg+"\u00B0"+pl.min+"'"}</span></div></div>})}
        </div>
      </div>}
      <div style={sCard}><h3 style={{fontFamily:F,fontSize:17,fontWeight:700,margin:"0 0 12px"}}>Check compatibility.</h3>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:12}}>
          {Object.entries(SIGNS).map(([k,s])=><button key={k} onClick={()=>setPartner(k)} style={{padding:"8px 4px",background:partner===k?C.pri+"20":C.soft,border:partner===k?"1px solid "+C.pri:"1px solid "+C.bor,borderRadius:8,cursor:"pointer",textAlign:"center",color:C.tx}}><div style={{fontSize:16}}>{s.sym}</div><div style={{fontFamily:F,fontSize:8,color:partner===k?C.pri:C.dim}}>{k}</div></button>)}
        </div>
        {partner&&rd&&(()=>{const c=getCompat(rd.sKey,partner);return <div className="fade" style={{textAlign:"center",padding:12}}><div style={{fontFamily:F,fontSize:38,fontWeight:800,color:c[0]>=80?C.grn:c[0]>=60?C.pri:C.red}}>{c[0]+"%"}</div><div style={{fontFamily:F,fontSize:14,fontWeight:600,marginBottom:8}}>{c[1]}</div></div>})()}
      </div>
    </div>}

    {/* AI CHAT */}
    {view==="chat"&&<div className="fade">
      <h2 style={{fontFamily:F,fontSize:20,fontWeight:700,margin:"0 0 4px"}}>AI Astrologer</h2>
      <p style={{fontFamily:F,fontSize:12,color:C.med,margin:"0 0 14px"}}>Your personal cosmic guide, powered by your birth chart.</p>
      {!user?<div style={{...sCard,textAlign:"center"}}><div style={{fontSize:32,marginBottom:8}}>{"\u2728"}</div><h3 style={{fontFamily:F,fontSize:16,fontWeight:700,margin:"0 0 8px"}}>Create an account to chat</h3><p style={{fontFamily:F,fontSize:12,color:C.med,margin:"0 0 14px"}}>The AI needs your birth chart for personalized readings.</p><button style={sBtn} onClick={()=>{setAuthMode("signup");setAuth(true)}}>Create account</button></div>
      :<div>
        <div style={{...sCard,padding:12,minHeight:300,maxHeight:"calc(100vh - 300px)",overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
          {chatMsgs.length===0&&<div style={{textAlign:"center",padding:"40px 20px",flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:28,marginBottom:8}}>{"\u2728"}</div><p style={{fontFamily:F,fontSize:13,color:C.med,margin:0}}>Ask me anything about your chart, love, career, or what the stars say about today.</p></div>}
          {chatMsgs.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"85%",padding:"10px 14px",borderRadius:m.role==="user"?"14px 14px 4px 14px":"14px 14px 14px 4px",background:m.role==="user"?C.pri+"20":C.soft,border:"1px solid "+(m.role==="user"?C.pri+"30":C.bor)}}><p style={{fontFamily:F,fontSize:13,lineHeight:1.55,margin:0,whiteSpace:"pre-wrap"}}>{m.text}</p></div></div>)}
          {chatLoading&&<div style={{display:"flex"}}><div style={{padding:"10px 14px",borderRadius:"14px 14px 14px 4px",background:C.soft,border:"1px solid "+C.bor}}><span className="typing" style={{fontFamily:F,fontSize:13,color:C.med}}>Reading the stars...</span></div></div>}
          <div ref={chatEnd}/>
        </div>
        <div style={{display:"flex",gap:8,marginTop:8}}><input value={chatIn} onChange={e=>setChatIn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat()}}} placeholder="Ask the stars..." style={{...sInp,flex:1}}/><button onClick={sendChat} disabled={chatLoading} style={{...sBtn,width:48,padding:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{"\u2192"}</button></div>
        <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>{["Today's energy?","My love life","Career guidance","Who am I?"].map((p,i)=><button key={i} onClick={()=>setChatIn(p)} style={{fontFamily:F,fontSize:10,padding:"6px 10px",background:C.soft,border:"1px solid "+C.bor,borderRadius:20,color:C.med,cursor:"pointer"}}>{p}</button>)}</div>
      </div>}
    </div>}

    {/* TOKEN READINGS */}
    {view==="tokens"&&<div className="fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div><h2 style={{fontFamily:F,fontSize:20,fontWeight:700,margin:0}}>Premium Readings</h2><p style={{fontFamily:F,fontSize:12,color:C.med,margin:"2px 0 0"}}>Deep, personalized AI readings for your chart.</p></div>
        {user&&<div style={{display:"flex",alignItems:"center",gap:4,padding:"6px 14px",background:C.gold+"12",borderRadius:20,border:"1px solid "+C.gold+"25"}}><span style={{fontSize:12}}>{"\u2B50"}</span><span style={{fontFamily:F,fontSize:14,fontWeight:800,color:C.gold}}>{tokens}</span></div>}
      </div>

      {!user?<div style={{...sCard,textAlign:"center"}}><div style={{fontSize:32,marginBottom:8}}>{"\u{1F52E}"}</div><h3 style={{fontFamily:F,fontSize:16,fontWeight:700,margin:"0 0 8px"}}>Create an account first</h3><p style={{fontFamily:F,fontSize:12,color:C.med,margin:"0 0 14px"}}>We need your birth chart to personalize your readings.</p><button style={sBtn} onClick={()=>{setAuthMode("signup");setAuth(true)}}>Create account</button></div>
      :<>
        {/* Active reading result */}
        {activeReading&&<div style={{...sCard,border:"1.5px solid "+C.gold+"40",background:"linear-gradient(135deg,"+C.card+" 0%,#1f1a14 100%)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{fontFamily:F,fontSize:14,fontWeight:700,color:C.gold}}>{activeReading.icon+" "+activeReading.name}</div>
            <button onClick={()=>{setActiveReading(null);setReadingResult(null)}} style={{fontFamily:F,fontSize:10,color:C.dim,background:"none",border:"none",cursor:"pointer"}}>{"\u2715"}</button>
          </div>
          {readingLoading?<div style={{textAlign:"center",padding:20}}><div style={{width:22,height:22,border:"2.5px solid "+C.bor,borderTopColor:C.gold,borderRadius:"50%",animation:"spin .7s linear infinite",margin:"0 auto 10px"}}/><p className="typing" style={{fontFamily:F,fontSize:12,color:C.med}}>Consulting the stars...</p></div>
          :readingResult&&<p style={{fontFamily:F,fontSize:13,lineHeight:1.65,margin:0,whiteSpace:"pre-wrap"}}>{readingResult}</p>}
        </div>}

        {/* Reading cards */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {READINGS.map(r=><div key={r.id} style={{...sCard,marginBottom:0,cursor:"pointer",position:"relative"}} onClick={()=>{if(tokens>=r.cost&&!readingLoading)buyReading(r)}}>
            <div style={{fontSize:24,marginBottom:6}}>{r.icon}</div>
            <div style={{fontFamily:F,fontSize:12,fontWeight:700,marginBottom:4}}>{r.name}</div>
            <p style={{fontFamily:F,fontSize:10,color:C.med,lineHeight:1.4,margin:"0 0 8px"}}>{r.desc}</p>
            <div style={{display:"flex",alignItems:"center",gap:3}}><span style={{fontSize:10}}>{"\u2B50"}</span><span style={{fontFamily:F,fontSize:11,fontWeight:700,color:tokens>=r.cost?C.gold:C.red}}>{r.cost} tokens</span></div>
            {tokens<r.cost&&<div style={{fontFamily:F,fontSize:9,color:C.red,marginTop:2}}>Need more tokens</div>}
          </div>)}
        </div>

        {/* Buy tokens */}
        <div style={{...sCard,marginTop:12}}>
          <div style={sLbl}>Buy Tokens</div>
          <p style={{fontFamily:F,fontSize:11,color:C.med,margin:"0 0 12px"}}>New accounts start with 3 free tokens. Get more to unlock deeper readings.</p>
          <div style={{display:"flex",gap:8}}>
            {PACKS.map((pk,i)=><div key={i} style={{flex:1,textAlign:"center",padding:12,borderRadius:10,background:pk.pop?C.pri+"10":C.soft,border:pk.pop?"1.5px solid "+C.pri:"1px solid "+C.bor,cursor:"pointer",position:"relative"}} onClick={()=>window.open(STRIPE_LINK,"_blank")}>
              {pk.pop&&<div style={{position:"absolute",top:-8,left:"50%",transform:"translateX(-50%)",fontFamily:F,fontSize:8,fontWeight:700,background:C.pri,color:C.bg,padding:"2px 8px",borderRadius:10}}>BEST VALUE</div>}
              <div style={{fontFamily:F,fontSize:20,fontWeight:800,color:C.gold}}>{pk.tokens}</div>
              <div style={{fontFamily:F,fontSize:9,color:C.dim,marginBottom:4}}>tokens</div>
              <div style={{fontFamily:F,fontSize:14,fontWeight:700,color:C.pri}}>{pk.price}</div>
            </div>)}
          </div>
        </div>

        {/* Subscription */}
        <div style={{...sCard,border:"1.5px solid "+C.gold+"30",background:"linear-gradient(135deg,"+C.card+" 0%,#1f1a14 100%)"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}><span>{"\u2728"}</span><span style={{fontFamily:F,fontSize:14,fontWeight:700,color:C.gold}}>Unlimited Plan</span></div>
          <p style={{fontFamily:F,fontSize:12,color:C.med,margin:"0 0 4px"}}>Unlimited AI chat + 10 tokens per month + all readings.</p>
          <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:10}}><span style={{fontFamily:F,fontSize:24,fontWeight:800,color:C.pri}}>$8</span><span style={{fontFamily:F,fontSize:11,color:C.dim}}>/month</span></div>
          <button style={{...sBtn,background:C.gold,color:"#000"}} onClick={()=>window.open(STRIPE_LINK,"_blank")}>Subscribe</button>
        </div>
      </>}
    </div>}
    ...
    {view==="learn"&&<div className="fade">
      ...
      {lSign&&(()=>{const s=SIGNS[lSign],el=EL[s.el];return <div style={sCard}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:32}}>{s.sym}</span><div><h3 style={{fontFamily:F,fontSize:20,fontWeight:700,margin:0}}>{lSign}</h3><div style={{display:"flex",gap:4,marginTop:3,flexWrap:"wrap"}}><span style={sTag(el.color)}>{el.icon+" "+el.name}</span><span style={sTag(C.acc)}>{s.ruler}</span></div></div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><div style={sSoft}><div style={sLbl}>In Love</div><p style={{fontFamily:F,fontSize:11,color:C.med,lineHeight:1.45,margin:0}}>{s.love}</p></div><div style={sSoft}><div style={sLbl}>At Work</div><p style={{fontFamily:F,fontSize:11,color:C.med,lineHeight:1.45,margin:0}}>{s.career}</p></div></div>
      </div>})()}
      {!lSign&&<>
        <div style={sCard}><div style={sLbl}>The 12 Signs</div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{Object.entries(SIGNS).map(([k,s])=><div key={k} onClick={()=>setLSign(k)} style={{textAlign:"center",padding:"12px 6px",borderRadius:10,background:EL[s.el].color+"06",border:"1px solid "+EL[s.el].color+"15",cursor:"pointer"}}><div style={{fontSize:22}}>{s.sym}</div><div style={{fontFamily:F,fontSize:11,fontWeight:700}}>{k}</div><div style={{fontFamily:F,fontSize:9,color:C.dim}}>{s.tag}</div></div>)}</div></div>
        <div style={sCard}><div style={sLbl}>The Planets</div>{Object.entries(PI).map(([k,p])=><div key={k} style={{...sSoft,display:"flex",alignItems:"center",gap:10,marginBottom:6}}><span style={{fontSize:18}}>{p.i}</span><div><div style={{fontFamily:F,fontSize:12,fontWeight:700}}>{p.n}</div><div style={{fontFamily:F,fontSize:10,color:C.med}}>{p.d}</div></div></div>)}</div>
      </>}
    </div>}
    ...
    {view==="profile"&&user&&(()=>{const s=SIGNS[user.signKey],el=EL[s.el];return <div className="fade">
      <div style={{...sCard,boxShadow:"0 0 20px rgba(88,166,255,0.08)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <div><h2 style={{fontFamily:F,fontSize:18,fontWeight:700,margin:0}}>{user.name}</h2><div style={{fontFamily:F,fontSize:11,color:C.dim,marginTop:1}}>{user.email}</div><div style={{display:"flex",gap:4,marginTop:3}}><span style={sTag(el.color)}>{s.sym+" "+user.signKey}</span></div></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:C.gold+"08",borderRadius:10,border:"1px solid "+C.gold+"20",marginBottom:8}}>
          <span style={{fontSize:16}}>{"\u2B50"}</span>
          <div><div style={{fontFamily:F,fontSize:14,fontWeight:800,color:C.gold}}>{tokens} tokens</div><div style={{fontFamily:F,fontSize:10,color:C.dim}}>available for premium readings</div></div>
          <button onClick={()=>go("tokens")} style={{marginLeft:"auto",fontFamily:F,fontSize:10,fontWeight:600,background:C.gold,color:"#000",border:"none",borderRadius:6,padding:"5px 10px",cursor:"pointer"}}>Get more</button>
        </div>
        {user.bday&&<div style={{padding:"8px 12px",background:C.gold+"08",borderRadius:8,border:"1px solid "+C.gold+"20",marginBottom:8}}><div style={{fontFamily:F,fontSize:11,fontWeight:700,color:C.gold}}>Birthday: {user.bday}</div></div>}
        <div style={sSoft}><div style={sLbl}>Your sign</div><p style={{fontFamily:F,fontSize:13,lineHeight:1.55,margin:0}}>{s.line}</p></div>
      </div>
      <div style={sCard}><div style={sLbl}>Your placements</div>
        {Object.entries(PI).map(([k,p])=>{const pl=user.chart[k];if(!pl)return null;const ps=SIGNS[pl.sign];const pe=EL[ps?.el]||EL.fire;return <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"8px 10px",background:pe.color+"06",borderRadius:8,border:"1px solid "+pe.color+"12",marginBottom:4}}><span style={{fontFamily:F,fontSize:12,color:C.med}}>{p.i+" "+p.n}</span><span style={{fontFamily:F,fontSize:12,fontWeight:700}}>{ps?.sym+" "+pl.sign}</span></div>})}
      </div>
      <button onClick={doLogout} style={{fontFamily:F,fontSize:12,fontWeight:600,background:"transparent",color:C.red,border:"1px solid "+C.red+"30",borderRadius:10,padding:"10px 20px",cursor:"pointer",width:"100%"}}>Log out</button>
    </div>})()}

    </div>

    {/* AUTH MODAL */}
    {auth&&<div onClick={()=>{setAuth(false);setAuthErr("")}} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.7)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} className="fade" style={{...sCard,width:"100%",maxWidth:380,marginBottom:0,maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",gap:0,marginBottom:14,borderRadius:8,overflow:"hidden",border:"1px solid "+C.bor}}>
          {[["signup","Create Account"],["login","Log In"]].map(([m,l])=><button key={m} onClick={()=>{setAuthMode(m);setAuthErr("")}} style={{flex:1,fontFamily:F,fontSize:12,fontWeight:600,padding:"9px 0",background:authMode===m?C.pri:"transparent",color:authMode===m?C.bg:C.dim,border:"none",cursor:"pointer"}}>{l}</button>)}
        </div>
        {authMode==="signup"&&<>
          <h3 style={{fontFamily:F,fontSize:17,fontWeight:700,margin:"0 0 3px"}}>Join Birth Chart Path</h3>
          <p style={{fontFamily:F,fontSize:11,color:C.med,margin:"0 0 4px"}}>Create your account and get <span style={{color:C.gold,fontWeight:700}}>3 free tokens</span> to try premium readings.</p>
          <input value={aNm} onChange={e=>setANm(e.target.value)} placeholder="First name" style={{...sInp,marginBottom:8}}/>
          <input value={aEm} onChange={e=>setAEm(e.target.value)} placeholder="Email" type="email" style={{...sInp,marginBottom:8}}/>
          <input value={aPw} onChange={e=>setAPw(e.target.value)} placeholder="Password (6+ characters)" type="password" style={{...sInp,marginBottom:8}}/>
          <div style={{display:"flex",gap:6,marginBottom:8}}><select value={aMo} onChange={e=>setAMo(e.target.value)} style={sSel}><option value="">Month</option>{MO.map(m=><option key={m}>{m}</option>)}</select><select value={aDy} onChange={e=>setADy(e.target.value)} style={sSel}><option value="">Day</option>{Array.from({length:31},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select></div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:8}}><select value={aHr} onChange={e=>setAHr(e.target.value)} style={sSel}><option value="">Hour</option>{Array.from({length:12},(_,i)=><option key={i+1} value={String(i+1)}>{i+1}</option>)}</select><select value={aAp} onChange={e=>setAAp(e.target.value)} style={{...sSel,maxWidth:80}}><option value="AM">AM</option><option value="PM">PM</option></select></div>
          <CityInput inputRef={acity}/>
          {authErr&&<p style={{fontFamily:F,fontSize:11,color:C.red,margin:"8px 0 0"}}>{authErr}</p>}
          <button style={{...sBtn,marginTop:12}} onClick={doSignup}>Create my account</button>
        </>}
        {authMode==="login"&&<>
          <h3 style={{fontFamily:F,fontSize:17,fontWeight:700,margin:"0 0 3px"}}>Welcome back</h3>
          <p style={{fontFamily:F,fontSize:11,color:C.med,margin:"0 0 14px"}}>Log in to your account.</p>
          <input value={aEm} onChange={e=>setAEm(e.target.value)} placeholder="Email" type="email" style={{...sInp,marginBottom:8}}/>
          <input value={aPw} onChange={e=>setAPw(e.target.value)} placeholder="Password" type="password" style={{...sInp,marginBottom:8}} onKeyDown={e=>{if(e.key==="Enter")doLogin()}}/>
          {authErr&&<p style={{fontFamily:F,fontSize:11,color:C.red,margin:"8px 0 0"}}>{authErr}</p>}
          <button style={{...sBtn,marginTop:12}} onClick={doLogin}>Log in</button>
        </>}
      </div>
    </div>}
  </div>;
